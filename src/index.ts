import badwordsData from "./data/badword.json";

type BadwordItem = any;
...
const flattened = (() => {
  ...
  for (const item of (badwordsData as any[])) {
    ...
    if (item.romanized) add(item.romanized);
    if (Array.isArray(item.variants)) {
      for (const v of item.variants) add(v);
    }
    if (Array.isArray(item.nepali)) {
      for (const v of item.nepali) add(v);
    }
    // Some entries use other keys like 'nepali' being a string
    if (typeof item.nepali === "string") add(item.nepali);
  }

  // Deduplicate by normalized text (keep first occurrence metadata)
  const seen = new Map<string, { text: string; base?: string; category?: string[]; raw?: BadwordItem }>();
...
export type MatchOptions = {
  ...
  maxDistance?: number; // absolute levenshtein distance allowed
  maxRatio?: number; // distance / len <= ratio
};

const DEFAULT_OPTS: Required<MatchOptions> = { fuzzy: false, maxDistance: 1, maxRatio: 0.25 };

export function containsProfanity(input: unknown, opts?: MatchOptions): boolean {
  ...
  for (const entry of flattened) {
    try {
      if (entry.testRegex.test(text)) return true;
      // also check normalized form as substring
      if (entry.normalized && normalizedText.includes(entry.normalized)) return true;
    } finally {
      (entry.testRegex as RegExp).lastIndex = 0;
    }
  }
  ...
  if (options.fuzzy) {
    ...
    while ((m = tokenRegex.exec(text)) !== null) {
      ...
      for (const entry of flattened) {
        ...
        if (Math.abs(len - Array.from(tok).length) > Math.max(2, Math.floor(len * options.maxRatio))) continue;
        const dist = levenshtein(tok, target);
        if (dist <= options.maxDistance || dist / Math.max(1, len) <= options.maxRatio) return true;
      }
    }
  }
...
export function findProfanities(input: unknown, opts?: MatchOptions): Array<{ match: string; base?: string; category?: string[]; index: number; fuzzy?: boolean }> {
  const options = { ...DEFAULT_OPTS, ...(opts || {}) };
  const text = String(input || "");
  const results: Array<{ match: string; base?: string; category?: string[]; index: number; fuzzy?: boolean }> = [];
...
export type CensorOptions = MatchOptions & { fuzzyCensor?: boolean };

export function censorProfanity(input: unknown, maskChar = "*", opts?: CensorOptions): string {
  ...
  const options = { ...DEFAULT_OPTS, ...(opts || {}) } as Required<CensorOptions>;
  if (options.fuzzy && options.fuzzyCensor) {
    ...
    if (matches.length > 0) {
      ...
      const spans: Array<{ start: number; end: number; mask: string }> = [];
      ...
      for (const s of spans) {
        ...
        if (appliedRanges.some(r => !(s.end <= r.start || s.start >= r.end))) continue;
